...
function getCountryByIso3(isoCode){
    ...
    let country = countries.find(element => element.iso3 == isoCode);
    return country;  
}
...
function getCountryTranslatedName(isoCode, language){
    //Complete with your code
    let getCountry = getCountryByIso3(isoCode);
    let translatedName = getCountry.translations[language];
...
function getCountriesBySubregion(subregion){
    // //Complete with your code
    let countriesSubregion = Array();
    function addForRegion(country){
        if(country.subregion == subregion){
            countriesSubregion.push(country);
        }
    }
    countries.forEach(addForRegion); 
